...
Raw CSV (6.3M rows)
   │
   ▼
ingest_bronze.py        chunked read → Parquet → Azure Data Lake
   │
   ▼
silver_transform.py     PySpark clean + dedupe → Delta table
   │
   ▼
gold_aggregations.py    fraud summary + risk watchlist → Delta tables
```
...
Needs an Azure Storage connection string in a local `.env` file to run the ingestion step (not included, see `.gitignore`).

## Dataset

[PaySim — synthetic financial dataset](https://www.kaggle.com/datasets/ealaxi/paysim1), Kaggle.
